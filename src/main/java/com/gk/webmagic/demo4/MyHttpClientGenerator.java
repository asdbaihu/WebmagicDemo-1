...
public class MyHttpClientGenerator extends HttpClientGenerator {
	...
	private boolean proxyFlag = false;//是否需要代理
	
	private PoolingHttpClientConnectionManager connectionManager;
	
	private MyHttpRoutePlanner planner = new MyHttpRoutePlanner();
	...
	public MyHttpClientGenerator(boolean proxyFlag) {
		Registry<ConnectionSocketFactory> reg = RegistryBuilder.<ConnectionSocketFactory>create()
                .register("http", PlainConnectionSocketFactory.INSTANCE)
                .register("https", buildSSLConnectionSocketFactory())
                .build();
	...
	public CloseableHttpClient getClient(Site site, Proxy proxy) {
		if(proxyFlag) {
			return getClient(site);
		}
		return super.getClient(site, proxy);
	}
